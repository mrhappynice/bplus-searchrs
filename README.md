...
#### Super small, super fast, Rust + HTML/JS UI. Local database of convos for research memory, provider and model selection(local and paid) , built-in native connectors and generic user added apis through UI. Optional connection to SearXNG. <sub>app might be a bit buggy :)</sub> added support in [bplus-TUI](https://github.com/mrhappynice/bplus-tui) | Launch with low resource WebView web-wraper similar to Tauri called [bplus-webwrap](https://github.com/mrhappynice/bplus-webwrap) simply command ```./webwrap searchrs``` 
---
### Quick Start🏁
